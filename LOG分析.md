...
原因是 `route.ts` 裡 `path.join(process.cwd(), "data", "visitor-stats.json")` 這種動態
路徑組合，讓 Turbopack 的 Node File Trace 分析器無法靜態確定範圍，保守地把整個專案都
標記為這支 API route 的依賴。這不影響這個專案的部署方式（沒有用 `output: "standalone"`，
build 本身仍然成功），但放著不管的話，以後真的有別的警告跳出來也會被這個「已知」警告
淹沒。已加上 `turbopackIgnore` 註解修掉，重新 build 確認警告消失。
